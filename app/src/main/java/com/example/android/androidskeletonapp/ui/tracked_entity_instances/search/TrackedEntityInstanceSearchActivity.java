...
public class TrackedEntityInstanceSearchActivity extends ListActivity {
    ...
    private void searchTrackedEntityInstances() {
        ...
        // TODO Get  of SEARCH root organisation units
        List<OrganisationUnit> organisationUnits = Sdk.d2().organisationUnitModule()
                .organisationUnits.byOrganisationUnitScope(OrganisationUnit.Scope.SCOPE_TEI_SEARCH).byRootOrganisationUnit(true).get();

        // TODO Get first program with registration
        Program program = Sdk.d2().programModule().programs.byProgramType().eq(ProgramType.WITH_REGISTRATION).get().get(0);

        // TODO Get TrackedEntityAttribute with name equal to "Malaria patient id"
        TrackedEntityAttribute attribute = null;
        attribute = Sdk.d2().trackedEntityModule().trackedEntityAttributes.byName().eq("Malaria patient id").one().get();
